...
export default function SelectPlan() {
  ...
  const { data, isFetching, isLoading, error, isRefetching, refetch } = useQuery<UserTrainingPlanData[]>({
    queryFn: async () => {
      const res = await Api.get<{ plans: UserTrainingPlanData[] }>('/user/training-plan', { params: { uid: user?._id, li: 10, p: 1 } })
    ...
    <View className='flex-1 w-full'>
      <View className='flex-row items-center mt-2'>
        ...
        <View className='flex-1 items-center mr-7'>
          <Text className='text-xl font-semibold'>Selecionar Plano de Treino</Text>
        </View>
      </View>
          ...
          return <>
            ...
            >
              ...
              <View className='flex-col items-start'>
                ...
                <View>
                  <Text className='text-xs text-slate-400'>{dayjs(item.createdAt).format('[Criado em] DD [de] MMMM [de] YYYY')}</Text>
                </View>
              </View>
            </Pressable>
