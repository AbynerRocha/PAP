...
export default function Header() {
    ...
    return (
        <View className='w-full h-20 flex-row items-center space-x-3'>
            {user ? <>
                <Pressable
                    className='w-14 h-14 rounded-full bg-slate-200 items-center justify-center ml-4 mt-5'
                    onLongPress={() => {
                ...
                <View className='mt-4'>
                    ...
                    <View className='flex-row space-x-1 items-center'>
                        <Text className='text-slate-900 text-xl font-bold'>{user.name.includes(" ") ? user.name.split(' ')[0] : user.name}</Text>
