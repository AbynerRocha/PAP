...
async function handler(req: FastifyRequest<Request>, rep: FastifyReply) {
    ...
    const user = await User.findOne({ email }).catch((err) => rep.status(500).send({ error: err.name, message: 'Não foi possivel realizar está ação. Tente novamente mais tarde.' }))
