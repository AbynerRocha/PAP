...
export default function Home() {
  ...
  function handleSelectDay(weekDay: number, newWorkout: WorkoutData | null) {
        ...
        {planSelected
          ? <View className='mt-4'>
                ...
                {planSelected.plan.map((plan, idx) => {
                  ...
                  >
                    <Text className={twMerge(
                      'font-semibold',
                      (daySelected === plan.weekDay && 'text-slate-50') 
                    )}>{weekDayName}</Text>
                  </Pressable>
                })}
              </ScrollView>
            ...
            <View className='flex-1 mt-2 items-center '>
              ...
              >
                <View className='h-full w-[80%] justify-center ml-3'>
                  <Text className='text-slate-50 text-2xl font-medium italic'>{workout.name.length > 30 ? workout.name.slice(0, 30)+'...' : workout.name}</Text>
                  <Text className='text-slate-50'>{workout.exercises.length} exercícios</Text>
                </View>
                <View>
            ...
            : <View className='bg-blue-800 h-24 w-[85%] mx-5 rounded-xl justify-center items-center p-3'>
              <Text className='text-slate-50 text-2xl font-semibold italic'>Descanso</Text>
            </View>
            }
            </View>
          </View>
